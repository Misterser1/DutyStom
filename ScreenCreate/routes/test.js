...
router.post('/console-errors', express.json(), async (req, res) => {
  try {
    ...
    const { includeWarnings = false, clear = false } = req.body || {};

    const result = await testPage.evaluate((opts) => {
      const errors = window.__consoleErrors || [];
      const warnings = opts.includeWarnings ? (window.__consoleWarnings || []) : [];
